...
void init()
{
    ...
    fmtlog::preallocate();
    // Poll every 100 microseconds (100,000 ns) in background thread
...
inline void execute_log(Scenario scenario, size_t i, const std::string& str)
{
    ...
    case Scenario::TwoInts:
        logi("Benchmark logging two integers: {} and {}",
             static_cast<int>(i),
             static_cast<int>(i * 2));
        break;
    ...
    case Scenario::StdString:
        logi("Benchmark logging std::string: {}", str);
        break;
...
LatencyStats run(Scenario scenario, size_t iters, size_t warmup_iters)
{
    ...
    double batch_time_sec = std::chrono::duration<double>(t_end - t_start).count();
    ...
    return LatencyStats::calculate("fmtlog", scenario_to_string(scenario),
                                   batch_time_sec, raw_cycles);
}
