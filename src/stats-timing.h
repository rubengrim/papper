...
namespace Target
{
/* CPU model name; here since we need to extract it anyway for Intel rdtsc time
 * clock rate.
 */
...
std::string CPUModelName()
{
    ...
    // Try the AArch64 MIDR_EL1 register; we might be able to work it out from
    // there.
...
#else
    // Add architecture specific code here if you can find appropriate things
    // to use. On Arm32, there are some registers, but they're not accessible
...
statistic& statistic::operator+=(const statistic& other)
{
    ...
    // Try to order these calculations to avoid overflows. If this were
    // Fortran, then the compiler would not be able to re-order over brackets.
    // In C++ it may be legal to do that (we certainly hope it doesn't, and the
    // C+ Programming Language 2nd edition suggests it shouldn't, since it says
    // that exploitation of associativity can only be made if the operation
...
/* Timing functions */
/* Since we are trying to use the highest resolution, lowest overhead, timer
 * which we can, this is target architecture dependent. On some architectures,
 * such as AARCH64 it is relatively easy; on others, such as X86_64, there is a
 * degree of complexity involved.
 */
double tsc_tick_count::TickTime = 0.0;
std::string tsc_tick_count::Description = "";

// Try to see whether the clock actually ticks at the same rate as its value is
// enumerated in. Consider a clock whose value is enumerated in seconds, but
// which only changes once an hour... Just because a clock has a fine interval,
// that doesn't mean it can measure to that level.
...
// Extract the tcik time from CPUID information; this is way more complicated
// than you would hope, and depends on the hardware vendor.
//
// For Intel there are two ways to do it :-
// 1) Use cpuid leaf 15h. This is best, but many cores don't yet have leaf 15h.
// 2) Read it from the CPU Model name (extracted from cpuid). Intel put the
// nominal frequency in the name,
//    so we can parse that.
//
// For AMD processors there seems to be no way to find this.
// They do not support leaf 15h and do not give the nominal frequency in the
// brand name. So we have to look at how many rdtsc ticks pass in a known
// amount of time (using std::chrono::steady_clock for the fixed time).
//
...
// 3) They do give a nominal frequency in the brand string, *BUT* that doesn't
// give the emulated rdtsc
//    tick period.
// So we have to recognize that we're on an Apple emulation and use the
// "measure it" approach.
//
...
static bool extractLeaf15H(double* time)
{
    ...
    *time = cpuinfo.eax / (cpuinfo.ebx * coreCrystalFreq);
    ...
    tsc_tick_count::setDescription(desc.str());
    // errPrintf("Compute rdtsc tick time: coreCrystal = %g, eax=%u, ebx=%u,
    // ecx=%u "
    //           "=> %5.2fps\n",
...
// We can reasonably put up with 5ms of overhead once at startup, and this
// should overceom any real danger of this being interrupted and so misleading.
static double measureTSCtick(char const* warning)
{
    ...
    std::stringstream desc;
    desc << "X86 TSC " << formatTimer(minTick) << " from measurement"
         << warning;
    tsc_tick_count::setDescription(desc.str());
    return minTick;
}
...
static double readHWTickTime()
{
    ...
    if (cpuinfo.eax >= 0x80000007)
    {
        ...
        if ((cpuinfo.edx & (1 << 8)))
        {
            ...
            // Try to get it from the brand name of the CPU (Intel have it
            // there as a string), and do seem to have the TSC clock run at the
            // notional CPU frequency. Be careful of Apple's emulation, though.
            // It claims to be "GenuineIntel", but the clock frequency it
            // announces in the model name is *not* the rate at which the
            // emulated TSC runs!
            ...
            // OK, we have invariant TSC, but we're on AMD or Apple emulation
            // or..., all we can do is measure it.
            return measureTSCtick("");
        }
    }
    // Either it can't tell us, or the CPU doesn't admit to having invariant
    // TSC. In that case, timing using TSC is dubious at best. However... there
    // seem to be some VMs (e.g. on Windows), which do that. So we'll assume
    // that they do, really, have invariant TSC, but are just not admitting it.
    //
...
#endif /* Architectures */
#else
/* Target does not have code to support a high-resolution low cost hardware
 * timestamp */
static double readHWTickTime()
{
...
::std::string formatSI(double interval, int width, char unit)
{
    ...
    if (useSI)
    {
        ...
        static struct
        {
            double scale;
            char prefix;
        } ranges[] = { { 1.e21, 'y' },  { 1.e18, 'z' },  { 1.e15, 'a' },
                       { 1.e12, 'f' },  { 1.e9, 'p' },   { 1.e6, 'n' },
                       { 1.e3, 'u' },   { 1.0, 'm' },    { 1.e-3, ' ' },
                       { 1.e-6, 'k' },  { 1.e-9, 'M' },  { 1.e-12, 'G' },
                       { 1.e-15, 'T' }, { 1.e-18, 'P' }, { 1.e-21, 'E' },
                       { 1.e-24, 'Z' }, { 1.e-27, 'Y' } };

        if (interval == 0.0)
        {
            os << ::std::setw(width - 3) << ::std::right << "0.00"
               << ::std::setw(3) << unit;
            return os.str();
        }
        ...
        for (int i = 0; i < (int)(sizeof(ranges) / sizeof(ranges[0])); i++)
        {
            if (interval * ranges[i].scale < 1.e0)
            {
                ...
                os << ::std::fixed << ::std::setprecision(2)
                   << ::std::setw(width - 3) << ::std::right
                   << (negative ? -interval : interval) << ::std::setw(2)
